...
int HexAI::checkWinner(PlayerEnum &winner)
{
    //get nodes for both blue and red
    vector<int> tmpvec;
    m_hex.getColorNodes(PlayerEnum::PLAYER_RED, tmpvec);
    unordered_set<int> rednodes(tmpvec.begin(), tmpvec.end());
    tmpvec.clear();

    m_hex.getColorNodes(PlayerEnum::PLAYER_BLUE, tmpvec);
    ...
    const unordered_set<int> &rednodes, PlayerEnum &winner)
{
    ...
    if (blueuf.connected(vbluenodes.first, vbluenodes.second) )
        ...
        winner = PlayerEnum::PLAYER_BLANK;

    return StatEnum::OK;
}

//for player computes move with most winning chance usign Monte Carlo simulation
int HexAI::computeMove(PlayerEnum player, int& movenodeid)
{
    ...
    m_hex.getColorNodes(PlayerEnum::PLAYER_BLANK, bnodeidvec);
    ...
    for( int movenodeid : bnodeidvec )
    {
        ...
        if (PlayerEnum::PLAYER_RED == player)
            rednodes.insert(movenodeid);  //insert the move 
        else if (PlayerEnum::PLAYER_BLUE == player)
        ...
        for (int i=0; i<NUM_REPS; ++i)
        {
            ...
            //  for both players
            PlayerEnum cplayer = player;
            for (int randnodeid : blanknodes )
            {
                cplayer = getOtherPlayer(cplayer);
                if (PlayerEnum::PLAYER_RED == cplayer)
                    rnodes.insert(randnodeid);
                else if (PlayerEnum::PLAYER_BLUE == cplayer)
            ...
            assert(winner != PlayerEnum::PLAYER_BLANK); //in full board there has to be a winner
